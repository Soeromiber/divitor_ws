  ...
  public:
    ...
    ThermalDetector(float tMin = 31.0f, float tMax = 38.0f,
        ...
        : minHumanTemp(tMin), maxHumanTemp(tMax), minContourArea(minArea),
          maxContourArea(maxArea), minAspectRatio(minAR),
    ...
    std::vector<HumanDetection> Detect(const float *rawMatrix,
        ...
        // thermal colormap
        cv::Mat normMat;
        cv::normalize(thermalUpscaled, normMat, 0, 255, cv::NORM_MINMAX,
                      CV_8UC1);
        ...
        for (const auto &contour : contours) {
            ...
            if (aspectRatio >= minAspectRatio &&
                ...
                cv::putText(outputVis, label,
                            cv::Point(bbox.x, std::max(bbox.y - 5, 15)),
...
class ThermalDetectorNode : public rclcpp::Node {
  public:
    const cv::Size UPSCALE_SIZE = cv::Size(
        320, 240); // Upscaled image size for detection and visualization
